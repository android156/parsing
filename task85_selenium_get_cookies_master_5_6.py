...
with webdriver.Chrome() as webdriver:
    ...
    for hacker_id, cookie in enumerate(cookies):
        webdriver.add_cookie(cookie)
        webdriver.refresh()
        time.sleep(0.5)
        li = webdriver.find_elements(By.CSS_SELECTOR, 'ul#skillsList li')
        num_skills = len(li)
        try:
            age = int(webdriver.find_element(By.CSS_SELECTOR, 'span#age').text.split(':')[-1].strip())
        except ValueError:
            print(f"{webdriver.find_element(By.CSS_SELECTOR, 'span#age').text} - тут ошибка определения возраста")
        value = cookie.get('value')
        hackers_list.append((age, num_skills, value))
        print(f'№{hacker_id} Возраст: {age}, Умений: {num_skills}, Код: {value}')
        webdriver.delete_all_cookies()

sorted_hackers_list = sorted(hackers_list, key=lambda x: (-x[0], x[1]))
print(sorted_hackers_list)
