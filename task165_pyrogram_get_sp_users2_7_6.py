...
def get_clear_message_length(message: str) -> int:
    if message:
        return len(message[:].replace(' ', '').replace("\n", ""))
    return 0
...
def main():
    ...
    results_list = []
    with app:
        messages = app.get_chat_history(chat_id=chat_name, )
        # members = app.get_chat_members(chat_id=chat_name, )
        ...
        for message in list(messages):
            if message.id % 2 == 0 and message.from_user and message.from_user.id % 2 == 0 and get_clear_message_length(
                    message.text) % 2 == 0:
                print('Сообщение: ', message.id, message.text, message.from_user.id)
                results_list.append(message.id * get_clear_message_length(message.text) * message.from_user.id)
