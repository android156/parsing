...
filter=MessagesFilter.ANIMATION            # Тип фильтра (например, 'photo' для фото)
    ...
    with app:
        ...
        for message in messages:
            if message.text and message.text.startswith(query):
                print(f'message id: {message.id} from user: {message.from_user.id}, '
                      f'message text: {message.text}')
            sum += message.id * message.from_user.id
        print(f'sum: {sum}')
