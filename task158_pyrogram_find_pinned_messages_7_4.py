...
def get_non_empty_sym_len(message):
    non_empty_sym_len = 0
    for sym in message:
        if sym not in [' ', '']:
            non_empty_sym_len += 1
    return non_empty_sym_len
...
filter=MessagesFilter.PINNED            # Тип фильтра (например, 'photo' для фото)
    ...
    with app:
        ...
        for message in messages:
            if message.text:
                ...
                sum += get_non_empty_sym_len(message.text)
        print(f'sum: {sum}')
